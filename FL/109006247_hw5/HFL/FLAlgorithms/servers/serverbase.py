...
class Server:
    ...
    def aggregate_parameters(self):
        ...
        #print(f"observe server paramter {self.selected_users[0].train_samples}")

        #initiate
        samples_sum = sum(user.train_samples for user in self.selected_users)
        #server_para_dict = {key: torch.zeros_like(param) for key, param in self.model.state_dict().items()}
        first_user_w = self.selected_users[0].train_samples / samples_sum
        server_para_dict = {name: para.clone() * first_user_w for name, para in self.selected_users[0].model.state_dict().items()}

        #calculate weight
        for user in self.selected_users[1:]:
            user_para_dict = user.model.state_dict()
            user_w = user.train_samples / samples_sum
            #add up
            for name, para in user_para_dict.items():
                #print(f"try to add up parameter {name}")
                server_para_dict[name] += user_w*para

        #finally load parameter to server model
        self.model.load_state_dict(server_para_dict)
        #print("done aggregated parameter")
    ...
    def select_users(self, round, num_users):
        ...
        if num_users > len(self.users):
            print("number of users to be selected exceed the existing users")
            return self.users
        else:
            #simply randomly sample user from self.users
