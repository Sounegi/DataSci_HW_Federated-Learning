...
class User:
    ...
    def set_parameters(self, model, beta=0.8):
        ## TODO
        '''
        ...
        '''
        #calculate the user model according to instruction
        '''print(f"observe user model {list(self.model.parameters())}") #look through the model
        print(f"observe global model {list(model.parameters())}")'''

        #self.model = beta*model + (1-beta)*self.model
        #go through each parameter
        user_para_dict = self.model.state_dict()
        global_para_dict = model.state_dict()
        for name, para in user_para_dict.items():
            if name not in global_para_dict:
                print("global parameter {name} mismatch")
            user_para_dict[name] = beta*global_para_dict[name] + (1+beta)*para
        
        #load para back to user's model
        self.model.load_state_dict(user_para_dict)

        '''#use list to speed up process
        user_parameters = list(self.model.parameters())
        global_parameters = list(model.parameters())

        for g_para, u_para in zip(global_parameters, user_parameters):
            u_para.data.mul_(1-beta).add_(g_para.data*beta)''' #no need since it slow at train

        #print("parameter settled")
